...
//            ).appendTo($this),
        ...
        var summarySize = (cutoutRadius - (doughnutRadius - cutoutRadius)) * 2.5,
        ...
        function pathClick(e) {
            var order = $(this).data().order;
            if (typeof data[order].action != 'undefined') data[order].action();
        }
        ...
        function shortKInt(int) {
            ...
            if (strlen < 5) return int;
            if (strlen < 8) return '<span title="' + int + '">' + int.substring(0, strlen - 3) + 'K</span>';
            return '<span title="' + int + '">' + int.substring(0, strlen - 6) + 'M</span>';
        }
        function getScaleFontSize(block, newText) {
            block.css('font-size', '');
            newText = newText.toString().replace(/(<([^>]+)>)/gi, '');
            ...
            var maxCharForDefaultFont = block.width() - (newText.length * block.css('font-size')) / settings.ratioFont;
            if (maxCharForDefaultFont < 0) return newFontSize + 'px';
            else return '';
        }
        /**
	function getScaleFontSize(block, newText) {
